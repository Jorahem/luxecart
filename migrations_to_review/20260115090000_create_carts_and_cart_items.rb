# db/migrate/20260115090500_create_orders_and_order_items.rb
class CreateOrdersAndOrderItems < ActiveRecord::Migration[6.0]
  def change
    # ------------------------
    # Orders table: create if missing, otherwise add any missing columns
    # ------------------------
    if table_exists?(:orders)
      # Add snapshot shipping fields if missing
      ...
      # Ensure status and payment_status columns exist
    ...
    if table_exists?(:order_items)
      ...
      # optionally ensure foreign keys exist (best-effort)
